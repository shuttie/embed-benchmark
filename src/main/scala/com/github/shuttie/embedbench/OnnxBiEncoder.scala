...
case class OnnxBiEncoder(
  ...
  def embed(batch: Array[String]): Array[Array[Float]] = {
    ...
    val tensorDim =
      Array(batch.length.toLong, encoded(0).getIds.length)
    val args = session.getInputNames.asScala.map {
      case n @ "input_ids" =>
        n -> OnnxTensor.createTensor(env, LongBuffer.wrap(tokens), tensorDim)
      case n @ "token_type_ids" =>
        n -> OnnxTensor.createTensor(
          env,
          LongBuffer.wrap(tokenTypes),
          tensorDim
        )
      case n @ "attention_mask" =>
